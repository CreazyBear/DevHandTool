...
{
    CIImage *image = [QRCodeImage qrByContent:content];
    ...
    return [QRCodeImage imageFromCGImageRef:scaledImage];
}
...
+ (NSImage *)qrImageWithContent:(NSString *)content size:(CGFloat)size red:(NSInteger)red green:(NSInteger)green blue:(NSInteger)blue {
    NSImage *image = [QRCodeImage qrImageWithContent:content size:size];
    ...
    CGContextDrawImage(context, CGRectMake(0, 0, imageWidth, imageHeight), [QRCodeImage nsImageToCGImageRef:image]);
    // 遍历像素
    ...
    NSImage *resultUIImage = [QRCodeImage imageFromCGImageRef:imageRef];
    // 清理空间
